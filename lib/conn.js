  ...
  constructor(argss) {
    ...
    (async () => {
      ...
      ig.fbns.on("direct_v2_message", async (m) => {
        ...
        if (m.message.includes("a post")) {
          // ig.media.info(m.actionParams.x);
          // console.log(await ig.feed.directInbox().request());
          // console.log(ig.feed.directThread(m.actionParams.id));
          console.log(ig.feed.directInbox(m.actionParams.id));
          // console.log(ig.entity.directThread(m.actionParams.x));
        }
